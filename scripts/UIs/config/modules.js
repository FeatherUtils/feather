...
uiManager.addUI(config.uinames.config.modules, 'config modules', (player) => {
    ...
    form.toggle(`${translate(config.lang.config.modules.toggles.ranks)}`, modules.get('cr'))
    form.toggle(`DevMode (Mostly unfinished or broken buttons)`, modules.get('devMode') ?? false)
    if(preview) form.divider()
    form.dropdown(`${translate(config.lang.config.modules.lang)}`, langs.map(_=>_.name), langs.findIndex(_=>_.val==modules.get('language')))

...
uiManager.addUI(config.uinames.basic.iconViewer, "Icon Viewer", async (player, page = 0, callbackFn, favoritedOnly = false, iconIDSearch = false, iconIDSearchError = false, defaultIconID = null)=>{
    ...
    if(iconIDSearch) {
        ...
        modalForm.textField("Icon ID", "Example: vanilla/apple", defaultIconID)
        modalForm.title(iconIDSearchError ? "§cIcon not found" : "Input Icon ID")
        modalForm.show(player, false, function (player, response) {
            if(response.canceled) return uiManager.open(player, config.uinames.basic.iconViewer, page, callbackFn, favoritedOnly);
            let iconID = response.formValues[0]
            if(!iconID || !icons.resolve(iconID)) {
                uiManager.open(player, config.uinames.basic.iconViewer, page, callbackFn, favoritedOnly, iconIDSearch, true, iconID ? iconID : null);
            } else {
        ...
        if(page === "PAGE_SELECT") {
            ...
            modalForm.show(player, false, (player, response)=>{
                if(response.canceled) {
                    uiManager.open(player, config.uinames.basic.iconViewer, 0, callbackFn, favoritedOnly);
                }
                if(response.formValues && response.formValues.length && /^\d+$/.test(response.formValues[0]) && parseInt(response.formValues[0]) > 0 && parseInt(response.formValues[0]) <= icons_.length) {
                    uiManager.open(player, config.uinames.basic.iconViewer, parseInt(response.formValues[0]) - 1, callbackFn, favoritedOnly);
                } else {
                    uiManager.open(player, config.uinames.basic.iconViewer, page, callbackFn, favoritedOnly);
                }
            })
            return;
        }
    ...
    for(let i = 0;i < 9 * 9;i++) {
        chest.button(i, `§cX`, [], `textures/blocks/tinted_glass`, 1, false, ()=>{
            uiManager.open(player, config.uinames.basic.iconViewer, page, callbackFn, favoritedOnly, iconIDSearch, iconIDSearchError, defaultIconID)
    ...
    chest.button(11 * rowCount, "§cBack", ["Go back 1 page"], "textures/blocks/glass_red", 1, false, ()=>{
        ...
        uiManager.open(player, config.uinames.basic.iconViewer, prevPage, callbackFn, favoritedOnly);
    });
    chest.button(callbackFn ? (11 * rowCount) + 3 : (11 * rowCount) + 4, "§dGo to page", ["Select a page to go to"], "textures/items/compass_item", 1, false, ()=>{
        uiManager.open(player, config.uinames.basic.iconViewer, "PAGE_SELECT", callbackFn, favoritedOnly);
    });
    ...
    if(callbackFn) {
        ...
        chest.button((11 * rowCount) + 4, "§dUse Icon ID", ["Manually input an icon ID"], "textures/items/spyglass", 1, false, ()=>{
            uiManager.open(player, config.uinames.basic.iconViewer, 0, callbackFn, favoritedOnly, true);
        });
        chest.button((11 * rowCount) + 6, "§eRecently Used", ["View recently used icons"], "textures/items/clock_item", 1, false, ()=>{
            uiManager.open(player, config.uinames.basic.iconViewer, 0, callbackFn, favoritedOnly == "RECENTLY_USED" || favoritedOnly == true ? false : "RECENTLY_USED");
        });
    ...
    chest.button((11 * rowCount) + 10, "§aNext", ["Go next 1 page"], "textures/blocks/glass_lime", 1, false, ()=>{
        ...
        uiManager.open(player, config.uinames.basic.iconViewer, nextPage, callbackFn, favoritedOnly);

...
world.beforeEvents.chatSend.subscribe(async e => {
    ...
    if (e.message.startsWith(".NETWORKING_LIB-COMPATIBLE-PLAYER:")) {
        ...
        if (!verified) {
            ...
            console.log(`Bot cant be verified: ${e.sender.name}`)
        } else {
            e.cancel = true;
            // e.cancel = true;
            http.setPlayer(e.sender);
            console.log(`Bot verified: ${e.sender.name}`)
        }
    }
    ...
    if (e.message == ".test") {
        e.cancel = true;
        if(http.player) e.sender.sendMessage('sending')
        http.makeRequest({
            method: 'get',
            url: 'https://nekos.life/api/v2/cat',
        }, (status, data) => {
            // world.sendMessage(`sent message`)
            e.sender.sendMessage(`Received a response from https://nekos.life/api/v2/cat with status ${status} and data:\n${data}`)
        })
    }
})
export default http